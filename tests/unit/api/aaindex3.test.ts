...
describe('GET /api/aaindex3', () => {
  ...
  it('body has database field set to aaindex3', () => {
    const res = makeRes()
    listHandler(makeReq(), res)
    expect((res.body as Record<string, unknown>).database).toBe('aaindex3')
  })

  it('returns all 47 records when no filter is applied', () => {
    const res = makeRes()
    listHandler(makeReq(), res)
    const body = res.body as { total: number; count: number; records: unknown[] }
    expect(body.total).toBe(47)
    expect(body.count).toBe(47)
    expect(body.records).toHaveLength(47)
  })
...
describe('GET /api/aaindex3/:accession', () => {
  ...
  it('body contains accession and database fields', () => {
    ...
    expect(body.database).toBe('aaindex3')
  })
